...
pub fn write_pair(original: &[u8], composited: &[u8], width: u32, height: u32) {
    ...
    let ok_original = write_png(&dir.join(format!("{stamp}-original.png")), original, width, height);
    let ok_composited = write_png(&dir.join(format!("{stamp}-composited.png")), composited, width, height);
    crate::log!(
...
fn write_png(path: &std::path::Path, rgba: &[u8], width: u32, height: u32) -> bool {
