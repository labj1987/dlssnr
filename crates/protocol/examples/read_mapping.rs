...
//! whether it's a valid dlssnr mapping. Used to verify true cross-toolchain interop --
...
fn main() {
    ...
    let hdr = unsafe { &*(map as *const dlssnr_protocol::ShmHeader) };
    println!("is_valid: {}", hdr.is_valid());
    println!("magic: {:#x} (expected {:#x})", hdr.magic.load(std::sync::atomic::Ordering::Relaxed), dlssnr_protocol::SHM_MAGIC);
    println!("version: {} (expected {})", hdr.version.load(std::sync::atomic::Ordering::Relaxed), dlssnr_protocol::SHM_VERSION);
