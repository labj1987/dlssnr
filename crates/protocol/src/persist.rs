...
mod tests {
    ...
    fn snapshot_then_apply_round_trips_every_setting() {
        let path = format!("{}/dlssnr-persist-test-{}/shm.bin", std::env::temp_dir().display(), std::process::id());
    ...
    fn apply_ignores_unknown_and_unparsable_keys() {
        let path = format!("{}/dlssnr-persist-test2-{}/shm.bin", std::env::temp_dir().display(), std::process::id());
        let m = mapping::open_at(&path).expect("failed to create test mapping");
        let h = m.header();
