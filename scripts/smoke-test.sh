...
cargo build -p dlssnr-layer

SCRATCH="$(mktemp -d)"
trap 'rm -rf "$SCRATCH"' EXIT

SO_PATH="$(pwd)/target/debug/libdlssnr_layer.so"
sed "s#\./libdlssnr_layer\.so#$SO_PATH#" data/VK_LAYER_dlssnr_neural.json \
    > "$SCRATCH/VK_LAYER_dlssnr_neural.json"
...
# enough (see data/VK_LAYER_dlssnr_neural.json and build-appimage.sh once that exists).
export VK_LAYER_PATH="$SCRATCH"
export VK_INSTANCE_LAYERS=VK_LAYER_dlssnr_neural
export VKLayer_DLSS5=1
export DLSSNR_UID="smoketest-$$"
export DLSSNR_LOG="$SCRATCH/layer.log"

echo "==> manifest: $SCRATCH/VK_LAYER_dlssnr_neural.json"
echo "==> layer library: $SO_PATH"
echo

cargo run --example smoke -p dlssnr-layer

echo
echo "==> layer log ($DLSSNR_LOG):"
cat "$DLSSNR_LOG" 2>/dev/null || echo "(no log written -- the layer never ran)"
